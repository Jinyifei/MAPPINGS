...
ADT 2017 08 07: Modified for HII region grids, reading in existing spectra
ADT 2020 06 03: Updated with improvements from shock grid, including ability
                to skip gridpoint dirs that already exist
"""
...
def gridpoint_worker(args):
    ...
    try:
        ...
        gridpt_name, gridpt_exists = initialise_gridpoint(Grid, gridpt_dict,
                                                    proc_num, logging_queue)
        if gridpt_exists:
        ...
        Z_frac = Grid.DF_abund_map.at[log_OH_12, "Z/Zsun"]
        spectrum_file = os.path.join(inputs_dir, "spectra",
                                "Spectrum_Zfrac{0:0.2f}.txt".format(Z_frac))
        if not os.path.exists(spectrum_file):
            raise ValueError("File not found: " + spectrum_file)
        ...
        while iter_num < n_P_iters:  # For all but the last iteration:
            ...
            # File suffix is "ph6" or "ph6" depending on whether we use P6 or P7
        ...
        # File suffixes are "ph6" or "ph6" depending on whether we use P6 or P7
        if n_P_iters > 0:
...
def initialise_gridpoint(Grid, gridpt_dict, proc_num, logging_queue):
    ...
    Returns (gridpoint_name, exists), where "exists" is True or False depending
    on whether the gridpoint dir already exists.  Initialisation is skipped if
    ...
    # Here is where we'd save out an OXAF spectrum if doing an NLR grid...

    return gridpoint_name, False
