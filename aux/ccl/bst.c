            ...
            struct libavl_allocator *allocator)
{
  ...
  if (compare != NULL) {
    ...
    tree = allocator->libavl_malloc (allocator, sizeof (struct bst_table));
    if (tree == NULL)
      return NULL;
...
bst_probe (struct bst_table *tree, void *item)
{
  ...
  int dir=1;                /* Side of |q| on which |p| is located. */
  ...
  if  ((tree != NULL) && (item != NULL)){
    ...
    n = tree->bst_alloc->libavl_malloc (tree->bst_alloc, sizeof(struct bst_node));
    if (n == NULL)
      return NULL;
...
{
  struct bst_node *p, *q;
  if ((tree != NULL) && (trav != NULL) && (item != NULL)) {
...
{
  if ((stack != NULL) && (height >= 0) && (new != NULL)) {
