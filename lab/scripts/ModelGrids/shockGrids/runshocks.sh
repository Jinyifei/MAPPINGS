...
########################################################################
#
# shock grid parameter - arg 2 can be changed
#
...
alpha="${2}"
#
# Magnetic EtaM, ratio of mag pressure to ram pressure.
# **** Edit V .mv script to use type 'R' field, preshock state
# New EtaM  = 2Pmag/Pram = 1/Ma^2
#            = 2Pmag/(mH*nH*v^2)
#            = B^2/(4*pi*n*mH*v^2) or using mu: B^2/(4*pi*n*mu*amu*v^2)
# etaM< is computed as -ve ie microG for each model
#
#EtaM="${2}"
#
# gas EtaG, ratio of gas pressure to ram pressure.
#
# New EtaG  = gammaPgas/Pram = 1/M^2
#            = gammaPgas/(mH*nH*v^2)
#            or using mu: gammaPgas/(n*mu*amu*v^2)
#
#EtaG="${2}"
#
# Alfven Mach Number v/va = sqrt(rho v^2/(2 Pmag))
# Adiabatic Mach Number v/vs = sqrt(rho v^2/(gamma Pgas))
#
#MachM="${2}"
#
########################################################################
...
if (( $# < 1 )); then
      ...
      echo " "
      echo " Usage2: runshocks.sh 'RunName' alpha n@100 n"
...
#
    vValue=$( awk  "BEGIN{ printf(\"%#.6f\",10.0^(1.0*${v0}+${idx}*${dv})) }" )
    vName=$( awk  "BEGIN{ printf(\"%0#6.1f\",10.0^(1.0*${v0}+${idx}*${dv})) }" )
#
...
#
# use EtaM directly with modified MAPPINGS script to use new R mag pressure option
#
