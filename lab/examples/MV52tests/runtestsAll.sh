...
echo " MV 5.2.0 Test 2: Primordial HII region ..."
date | awk '{print " "$0}' > timing.txt
echo " $HOST" >> timing.txt
...
echo " MV 5.2.0 Test 3: Test Solar Z 100kK PN ..."
date | awk '{print " "$0}' > timing.txt
echo " $HOST" >> timing.txt
...
echo " MV 5.2.0 Test 4: Dusty Isobaric Low Z HII region ..."
date | awk '{print " "$0}' > timing.txt
echo " $HOST" >> timing.txt
...
echo " MV 5.2.0 Test 5: Dusty Isobaric PAH HII region ..."
date | awk '{print " "$0}' > timing.txt
echo " $HOST" >> timing.txt
...
echo " MV 5.2.0 Test 6: 200 km/s iterative shock ..."
date | awk '{print " "$0}' > timing.txt
echo " $HOST" > timing.txt
(time "${mexe}" < "$s/06_shock200.mv" >>map5output.txt)| awk '{print " "$1" "$2" "$3" "$4}'>> timing.txt
cat timing.txt >> "$r/MV52test_$d.txt"
cat timing.txt
...
echo " MV 5.2.0 Tests completed. Output in "$r/MV52test_$d.txt""
# rm -f timing.txt map5output.txt
