...
.. container::
   ...
   .. rubric:: Downloading, installing, and data inputs
      :name: downloading-installing-and-data-inputs

See :doc:`installation` for how to get, build, and install MAPPINGS
(including compiler prerequisites and directory layout), and
:doc:`inputs` for the startup data files (``map.prefs``, abundance
files, atomic data) MAPPINGS reads before presenting the model menu.
The rest of this guide assumes you have already completed that step.
...
.. note::
   ...
   This section predates the current ``map52`` executable name (it was
   written for the ``map51`` release) and its sample script has not been
   re-verified against the current code. For a worked example that has
   been run and checked against current output, see
   :doc:`walkthrough_p6`. The exercise below is still a reasonable way
   to get a feel for the prompt sequence.

In the terminal, change to the /lab directory. Run the command ./map52 .
