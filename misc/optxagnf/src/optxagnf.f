...
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine optxagnf(ear, ne, param, ifl, total, disk, coro, nont)
      include 'const.inc'
      integer ne
      real*8 ear(0:ne),ph(ne),param(*)
      real*8 total(ne),disk(ne),coro(ne),nont(ne)
      integer n,ifl
...
c     always returns the total and all the components,
c     no need for inefficient repeated -ve parameter calls
c
      call mydiskf (ear, ne, param, ifl, total, disk, coro, nont)
c
      return
      end
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine mydiskf (ear, ne, param, ifl, tp, dk, co, no)
      include 'const.inc'
      integer ne,ifl
      real*8 ear(0:ne),param(*)
      real*8 tp(ne),dk(ne),co(ne),no(ne)
c
      real*8 m,mdot,rsg
      real*8 astar,da,z1,z2,rms,fcor,fpl
      real*8 corona,cor,pow,rcor,mytemp,fcol
      real*8 rgcm,t,t0,r,dr,dlogr,dflux
      real*8 el,ec,er,dfluxl,dfluxc,dfluxr
      real*8 dsk,tot,d0,d,logrout
      integer i,iin,imax,n
      logical first
      real*8 flux(ne),disc,discu,mdotedd,alpha,eff
...
c      write(*,*) 'eff= ',eff
c      write(*,*) 'rms= ',rms
c      mdot in g/s Ledd=1.39e38 m and c2=9e20 - no it isn!!!!! RSS
c      mdot=mdotedd*1.39d18*m/(9.0d0*eff)
c      mdot in g/s Ledd=1.2572e38 m and c2=8.98755e20
      mdot=lonledd*ledd*m/(c*c*eff)
c      write(*,*) eff, mdotedd, mdot
c     disc parameters
...
c         disc=disc+4.d0*pi*r*dr*rgcm*rgcm*5.67d-5*(t**4)
        disc=disc+da*stb*(t**4)
        t=t/kkev
c        go over each photon energy - 3rd order ppm integral average
        do n=1,ne,1
C          en=0.5d0*(dlog10(ear(n))+dlog10(ear(n-1)))
C          en=10.d0**en
          el = ear(n-1)
          ec = 0.5d0*(ear(n-1)+ear(n))
          er = ear(n)
c           do blackbody spectrum left edge
          if ((el.lt.30.d0*t*fcol).and.(r.gt.rcor)) then
c               dflux=pi*2.d0*h*((en*kevhz)**3)/9.0d20
            dfluxl=pi*2.d0*h*((el*kevhz)**3)/(c*c)
...
c               dflux=pi*2.d0*h*((en*kevhz)**3)/9.0d20
            dfluxc=pi*2.d0*h*((ec*kevhz)**3)/(c*c)
...
c               dflux=pi*2.d0*h*((en*kevhz)**3)/9.0d20
            dfluxr=pi*2.d0*h*((er*kevhz)**3)/(c*c)
...
c      write(*,*) lfrac,hfrac
      ...
      enddo
c
c renormalise to total spectrum to recover L/Ledd before excluding
c components
c
      tot=0.d0
      do n=1,ne,1
        ec=0.5d0*(ear(n-1)+ear(n))
        tot=tot+(dk(n)+co(n)+no(n))*ec
      enddo
...
c
      mytemp=3.d0*g*msun*m0*mdot0
      mytemp=mytemp/(8.d0*pi*stb*(r*rgcm)**3)
