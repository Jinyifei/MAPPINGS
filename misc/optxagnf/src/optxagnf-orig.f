...
c
c Standalone FORTRAN version of the xspec optxagnf model routine.
c
c Returns array photar of bin integrals: photons/cm^2/s per bin
c         *not differential* phots/cm^2/s/keV etc etc
c that is : bin width is already integrated --like all xspec additive
c model components.
c
c ear(i), in keV usually, is the energy at the right of the bin:
...
c
c so, approx:L(i) multiply photar(i) by ear(i) to get luminosity of the bin,
c and sum L(i) to get total luminosity.
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      subroutine optxagnf (ear, ne, param, ifl, photar)
      implicit none
      integer ne
      real*8 ear(0:ne),photar(ne),param(*)
      integer nn,n,i,ix,ifl
      parameter (nn=5000)
      real*8 e(0:nn),ph(nn)
      real*8 dloge,e1,e2,emin,emax,zfac
      save e,ph
...
c     set up own energy grid
      e(0)=dmin1(1.0d-3,ear(0)*zfac)
      e(nn)=dmax1(1.0d3,ear(ne)*zfac)
      dloge=dlog10(e(nn)/e(0))/dble(nn)
      do n=1,nn,1
        e(n)=10.d0**(dlog10(e(0))+dloge*dble(n))
      enddo
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 ear(0:ne),photar(ne),param(*)
      real*8 m,mdot,rsg
      real*8 astar,da,z1,z2,rms,fcor,fpl
      real*8 corona,cor,pow,rcor,mytemp,fcol
      real*8 rgcm,pi,t,t0,r,dr,dlogr,dflux
      real*8 en,tot,kkev,h,kevhz,d0,d,logrout
      real*8 hfrac,lfrac
      integer i,iin,imax,n
      logical first
      integer nn
      parameter (nn=5000)
      real*8 flux(nn),disc,discu,mdotedd,alpha,eff
      real*8 lpar(5),lphot(nn),lphote(nn)
      real*8 hpar(5),hphot(nn),hphote(nn)
c     constants
      h=6.62617d-27
      kkev=1.16048d7
      kevhz=2.417965d17
...
c     system parameters
      ...
      d0=param(2)!inmpc
      mdotedd=10.d0**(param(3))!if-ve plot disc
      astar=param(4)
      alpha=0.1d0
c
...
c     get rms
c
      z1=((1.d0-astar**2)**(1.d0/3.d0))
      z1=z1*(((1+astar)**(1.d0/3.d0))+((1-astar)**(1.d0/3.d0)))
      z1=1.d0+z1
      z2=dsqrt(3.d0*astar*astar+z1*z1)
      rms=3.d0+z2-dsqrt((3.d0-z1)*(3.d0+z1+2.0d0*z2))
      eff=1.0d0-dsqrt(1.0d0-2.0d0/(3.0d0*rms))

c     mdot in g/s Ledd=1.39e38 m and c2=9e20 - No it isn't - RSS
      mdot=mdotedd*1.39d18*m/(9.0*eff)
c      mdot=mdotedd*1.26d18*m/(9.0*eff)
...
c             write(*,*) 'seed photon',t,t0*kkev
        ...
        disc=disc+da*5.67d-5*(t**4)
        t=t/kkev
c        go over each photon energy - midpoint of bin - no it isn't!
        do n=1,ne,1
          en=0.5d0*(dlog10(ear(n))+dlog10(ear(n-1)))
          en=10.d0**en
c           do blackbody spectrum
          if ((en.lt.30.d0*t*fcol).and.(r.gt.rcor)) then
            dflux=pi*2.d0*h*((en*kevhz)**3)/9.0d20
            dflux=dflux*da/(dexp(en/(t*fcol))-1.d0)
            dflux=dflux/(fcol**4)
          else
            dflux=0.0d0
          endif
...
c        now multiply by energy band in Hz
        photar(n)=flux(n)*(ear(n)-ear(n-1))*kevhz
      enddo
      fcor=discu/(disc-discu)
...
c
      ...
      enddo
c      write(*,*) 't2',tot*kev*(4.d0*pi*d*d)
c
c     now add in the rest of the components
c
      lfrac=(tot/cor)*fcor*(1.d0-fpl)
      hfrac=(tot/pow)*fcor*fpl
c      write(*,*) lfrac,hfrac
c
      do n=1,ne,1
        if ((param(5).lt.0.d0).or.(param(8).lt.0.d0).or.(param(10)
     &   .lt.0.d0)) then
          if (param(10).lt.0.d0) photar(n)=hphot(n)*hfrac
          if (param(8).lt.0.d0) photar(n)=lphot(n)*lfrac
c            if (param(5).lt.0.d0) photar(n)=photar(n)
        else
          photar(n)=photar(n)+lphot(n)*lfrac+hphot(n)*hfrac
        endif
      enddo
c
      return
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      real*8 function mytemp(m0,astar,mdot0,rms,r0)
      include 'const.inc'
      real*8 m0,astar,mdot0,r,r0,rgcm,y,yms
      real*8 y1,y2,y3,part1,part2,part3,rms,a,b
      rgcm=1.5d5*m0   ! really?
c
      r=r0
      y=dsqrt(r0)
      yms=dsqrt(rms)
      y1=2.d0*dcos((dacos(astar)-pi)/3.d0)
      y2=2.d0*dcos((dacos(astar)+pi)/3.d0)
      y3=-2.d0*dcos((dacos(astar)/3.d0))
c
      part3=3.d0*((y3-astar)**2)*dlog((y-y3)/(yms-y3))
      part3=part3/(y*y3*(y3-y1)*(y3-y2))
      part2=3.d0*((y2-astar)**2)*dlog((y-y2)/(yms-y2))
      part2=part2/(y*y2*(y2-y1)*(y2-y3))
      part1=3.d0*((y1-astar)**2)*dlog((y-y1)/(yms-y1))
      part1=part1/(y*y1*(y1-y2)*(y1-y3))
      a=1.d0-yms/y-(3.d0*astar/(2.d0*y))*dlog(y/yms)-part1-part2-part3
      b=1.d0-3.d0/r+2.d0*astar/(r**1.5d0)
c
      mytemp=3.d0*6.67d-8*2.0d33*m0*mdot0
      mytemp=mytemp/(8.d0*pi*5.67d-5*(r*rgcm)**3)
...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      real*8 gamln
      integer itmax , n
      parameter (itmax=10000,eps=3.d-7)
