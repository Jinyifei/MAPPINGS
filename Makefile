...
#
# INSTALLBIN is the location of the mappings an tools executables]
...
# installation are created automatically for bash/zsh shells
# and can be found initially in ~/mappings520 and the user can edit their
# startup shell scripts: .bashrc (.zshrc) so mappings
...
help:
	...
	@echo ' '
	@echo "'make tools'      make and install tools programs"
	@echo ' '
	@echo '-----------------------------------------------------------'
#
#-----------------------------------------------------------
...
#
# compile and create input for .bashrc
#
${BINDIR}/${OUTNAME}: ${INCS} ${OBJ}
	...
	@echo '#############################################################'
	cd tools; $(MAKE) compile
	@echo ' '
	@echo '#############################################################'
	@echo ' A local version of the executable ${OUTNAME} can be found in:'
	@echo ' ${EXEDIR} . Install startup variables for .bashrc_profile'
	@echo ' or .bashrc with the contents of "for_bashrc.txt", '
	...
	@echo 'alias ${OUTNAME}="$$mapbin/${OUTNAME}"' >> for_bashrc.txt
	@echo 'alias m52="cd $$mapbase"'   >> for_bashrc.txt
	@echo '#' >> for_bashrc.txt
	@echo '########################################################################' >> for_bashrc.txt
#
#------------------------------------------------------------
...
install:
	...
	[ -e ${INSTALLBIN}/map52 ] || cp ${BINDIR}/${OUTNAME} ${INSTALLBIN}/map52
#
...
#
	...
	cp -r tools ${INSTALLDATA}/
	cd tools; $(MAKE) install
	cp -r docs ${INSTALLDATA}/
#
	rm -rf ${INSTALLDATA}/docs/doxygen
	rm -rf ${INSTALLDATA}/docs/doxygen_drafting
	rm -rf ${INSTALLDATA}/docs/art
...
#
	...
	@echo " Use "for_bashrc.txt" to edit  startup settings,"
	@echo " if needed, located in:"
	@echo "${INSTALLDATA}."
	@echo " "
	@echo "#############################################################"
