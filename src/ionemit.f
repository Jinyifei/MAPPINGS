...
c! Classica single multi-level atom model - test atomic data more
c! then of any practical modelling use despite community beliefs.
c!
c! @param This routine has no parameters
c!
c! @return
c!  Takes any of the 100+ multi-level ions and sets the temperaure
c!  or density and returns series of line (vac) emissivities
c!
c! @details
c!  Choose an ion and run an emmisivity test over te at const ne
c!  or over ne at constant te.
c***************************************************************
c
      subroutine ionemit
c
...
c
   ...
   30 format(//' Choose a single multi-level ion model type :'/
     ...
     &'       :  '/
     &'     P :  Fixed pressure P/k, vary density'/
     &'     Q :  Fixed pressure P/k, vary temperature'/
     &'  :: ',$ )
   40 write (*,30)
      read (*,20) ilgg
      ilgg=ilgg(1:1)
      call toup (ilgg, ilgg)
      if ((ilgg.ne.'A').and.(ilgg.ne.'B').and.(ilgg.ne.'P')
     &.and.(ilgg.ne.'Q')) goto 40
c
      modeltype=0
      if (ilgg.eq.'A') modeltype=0
      if (ilgg.eq.'B') modeltype=1
      if (ilgg.eq.'P') modeltype=2
      if (ilgg.eq.'Q') modeltype=3
c
...
c
c constant pressure models
c
c
      if (modeltype.eq.2) then
c
c modeltype.eq.2, Fixed P/k, Fixed Te
c
  490 format(//' Give fixed Pressure, variable Density:',/
     &'::::::::::::::::::::::::::::::::::::::::::::::::::::::::'/
     &'    log P/k ,P_H/k = nH.T (cgs log10 <= 12 linear > 12),'/
     &' :: ',$)
        write (*,490)
        read (*,*) phk
        if (phk.le.12.d0) phk=10.d0**phk
        cab=0.d0
  500 format(//' Give density (de = dh) variation:',/
     &'::::::::::::::::::::::::::::::::::::::::::::::::::::::::'/
     &'    dh_min, delta, n:'/
     &'    ( dh_min, delta taken as log10)'/
     &'    ( dh = 10**(dh_min + (delta)*i), i = 0..n)'/
...
c
  510 format (a)
  520 format(//' Give a name/code for this run: ',$)
        write (*,520)
        read (*,510) runname
        write (luop,10) theversion,runname
        write (*,*)
c
c Find t for phk and dh = dh_min
c
        dh=10.d0**dhmin
        t=phk/dh
  530 format (/' Transition Data at :',1pg11.3,' K',//
     &,' i, j,  Eij,         lamij,       Upsij,       Aji, ',/)
  540 format (/' Transition Data at :',1pg11.3,' K'//
     &,' i j  Eij         lamij       Upsij       Aji ',/)
  550 format (i2,',',i2,4(',',1pg14.7))
  560 format (i2,x,i2,4(x,1pg14.7))
        write (luop,530) t
        write (*,540) t
        nt=nfmtrans(idx)
        if (linetype.eq.0) then
          ...
          do itr=1,nt
            i=nfmlower(itr,idx)
            j=nfmupper(itr,idx)
            write (luop,550) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,
     &       fupsilontr(t,itr,idx),aim(j,i,idx)
            write (*,560) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,
     &       fupsilontr(t,itr,idx),aim(j,i,idx)
          enddo
        endif
        if (linetype.eq.2) then
          ...
          do itr=1,nt
            i=nfelower(itr,idx)
            j=nfeupper(itr,idx)
            write (luop,550) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,
     &       ffeupsilontr(t,itr,idx),aife(j,i,idx)
            write (*,560) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,
     &       ffeupsilontr(t,itr,idx),aife(j,i,idx)
          enddo
        endif
c
...
c
c modeltype.eq.3, Fixed P/k - Fixed ne/nh
c
  570 format(//' Give fixed Pressure, variable Temperature:',/
     &'::::::::::::::::::::::::::::::::::::::::::::::::::::::::'/
     &'    log P/k ,P_H/k = nH.T (cgs log10 <= 12 linear > 12),'/
     &' :: ',$)
        write (*,570)
        ...
        if (phk.le.12.d0) phk=10.d0**phk
        cab=0.d0
  580 format(//' Give Temperature variation:',/
     &'::::::::::::::::::::::::::::::::::::::::::::::::::::::::'/
     &'    T_min, delta, n:'/
     &'    ( T_min <= 10 as log, > 10 linear)'/
     &'    ( delta always taken as log10 )'/
...
c
  590 format (a)
  600 format(//' Give a name/code for this run: ',$)
        write (*,600)
        read (*,590) runname
        write (luop,10) theversion,runname
        write (*,*)
c
c Find dh for phk and t = 10^tmin, tmin is always in log loop, dhmax
c
        t=10.d0**tmin
        dh=phk/t
c
...
c Collisional Lines
c
  610 format (/' Transition Data :'//
     &  ,' i, j,  Eij,         lamij,       Aji, ',/)
  620 format (/' Transition Data :'//
     &  ,' i j  Eij         lamij       Aji ',/)
  630 format (i2,',',i2,3(',',1pe11.4))
  640 format (i2,i2,3(x,1pe11.4))
          write (luop,610)
          write (*,620)
          nt=nfmtrans(idx)
          if (linetype.eq.0) then
            ...
            do itr=1,nt
              i=nfmlower(itr,idx)
              j=nfmupper(itr,idx)
              write (luop,630) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,
     &         aim(j,i,idx)
              write (*,640) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,aim(j,
     &         i,idx)
            enddo
          endif
          if (linetype.eq.2) then
            ...
            do itr=1,nt
              i=nfelower(itr,idx)
              j=nfeupper(itr,idx)
              write (luop,630) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,
     &         aife(j,i,idx)
              write (*,640) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,
     &         aife(j,i,idx)
            enddo
          endif
c
...
c
          ...
          if (usekappa) write (luop,*) 'Kappa:',nt
c
  650 format(/,a,36(x,a13,','))
  660 format(a,36(x,1pg13.6,','))
  670 format(39(x,1pe13.6,','))
          if (linetype.eq.0) then
            do itr=1,nt
              s=nfmid(itr,idx)
              sl=len(s)
          ...
          do i=0,ntsteps
            ...
            if (linetype.eq.0) then
              do itr=1,nt
                ups(itr)=fupsilontr(t,itr,idx)
              enddo
              write (*,670) logt,t,de,(ups(itr),itr=1,nt)
              write (luop,670) logt,t,de,(ups(itr),itr=1,nt)
            endif
            if (linetype.eq.2) then
              do itr=1,nt
                ups(itr)=ffeupsilontr(t,itr,idx)
          ...
          do i=0,ntsteps
            ...
            if (linetype.eq.0) then
              do itr=1,nt
                br(itr)=fmbri(itr,idx)
              enddo
              write (*,670) logt,t,de,(br(itr),itr=1,nt)
              write (luop,670) logt,t,de,(br(itr),itr=1,nt)
            endif
            if (linetype.eq.2) then
              do itr=1,nt
                br(itr)=febri(itr,idx)
...
c Recombination
c
  680 format (/' H II Transitions :'//
     &,' Line, Series, lambda, Emiss. ',/)
  690 format (i3,', ',i3,', ',1pe14.6)
  700 format (i3,',',1pe14.6',',1pe14.6)
  710 format (201(1pe14.6,','))
c
          if (at.eq.zmap(1)) then
            write (luop,680)
            write (*,680)
            do series=1,nhseries
              do line=1,nhlines
                write (luop,690) line,series,hlambda(line,series)
              enddo
            enddo
          endif
c
  720 format (/' He II Transitions :'//
     &,' Line, Series, lambda, Emiss. ',/)
c
          if (at.eq.zmap(2)) then
            if (io.eq.2) then
              ...
              do series=1,nheseries
                do line=1,nhelines
                  write (luop,690) line,series,helambda(line,series)
                enddo
              enddo
            endif
          endif
c
  730 format (/' He I Transitions :'//
     &,' Line, lambda, Emiss. ',/)
c
          if (at.eq.zmap(2)) then
            if (io.eq.1) then
              ...
              write (*,*) ' HeI Legacy Lines:'
              do line=1,3
                write (luop,700) line,(heilam(line)*1.d8)
              enddo
              write (luop,*) ' HeI Singlets:'
              write (*,*) ' HeI Singlets:'
              do line=1,nheislines
                write (luop,700) line,heislam(line),heiseij(line)
              enddo
              write (luop,*) ' HeI Triplets:'
              write (*,*) ' HeI Triplets:'
              do line=1,nheitlines
                write (luop,700) line,heitlam(line),heiteij(line)
              enddo
            endif
          endif
c
  740 format (/' C II Transitions :'//
     &,' Line, lambda, Eij. ',/)
c
          if (at.eq.zmap(6)) then
            if (io.eq.2) then
              ...
              write (*,740)
              do line=1,nrccii
                write (luop,700) line,rccii_lam(line),rccii_eij(line)
              enddo
            endif
          endif
c
  750 format (/' N II Transitions :'//
     &,' Line, lambda, Eij. ',/)
c
          if (at.eq.zmap(7)) then
            if (io.eq.2) then
              ...
              write (*,750)
              do line=1,nrcnii
                write (luop,700) line,rcnii_lam(line),rcnii_eij(line)
              enddo
            endif
          endif
c
  760 format (/' O I Transitions :'//
     &,' Line, lambda, Eij. ',/)
c
          if (at.eq.zmap(8)) then
            if (io.eq.1) then
              ...
              write (*,760)
              do line=1,nrcoi_q
                write (luop,700) line,rcoi_qlam(line),rcoi_qeij(line)
              enddo
              do line=1,nrcoi_t
                write (luop,700) line,rcoi_tlam(line),rcoi_teij(line)
              enddo
            endif
          endif
c
  770 format (/' O II Transitions :'//
     &,' Line, lambda, Eij. ',/)
c
          if (at.eq.zmap(8)) then
            if (io.eq.2) then
              ...
              write (*,770)
              do line=1,nrcoii
                write (luop,700) line,rcoii_lam(line),rcoii_eij(line)
              enddo
            endif
          endif
c
c
          if (at.eq.zmap(10)) then
            if (io.eq.2) then
              do line=1,nrcneii
                write (luop,700) line,rcneii_lam(line),rcneii_eij(line)
              enddo
            endif
          endif
          do i=0,ntsteps
...
c
          ...
          do i=1,ntsteps
            ...
            dh=phk/t
            de=dh
            write (luop,710) t,(emiss(idx,i),idx=1,itr)
          enddo
          close (luop)
c
c Recombination Lines
...
c
c modeltype.eq.3, Fixed P/k - Fixed de
c
      endif
c
c
...
c! Explore the critical densities of lines
c! @param This routine has no parameters
c!
c! @return
c!  Table of line fluxes and ratios and de-excitation rate
c!
c! @details
c!    Subject a single ion to a temperature and density range,
c!    targeting two lines, and outputting de-excitation information
