...
c****************************************************************
c> @brief Entry to the data intialisation routines, some files some formulae
c! @param [in,out]  logical     error  Fatal error returned by a routine
c!
c! @return
c!  Error may or may not be returned if subroutine terminates itself
c!
c! @details
c!  Set of data intialisations and precalculation of ennergy bins
c! edge and centres plus photo crossections.  Order is important
c! switches for changing default behavious - eg using previous
c! recombination are first, with misc small arrays then the energy
c! bins are read from PHOTDAT so later inputs can be rebinned, then
c! in order for inisation etc.  Then lines in bins to find min bin
c! numbers for bins and edges to minimise do loops thoughout.
c! finally some diagnostic printouts can be added and used esp in
c! expoertmode = 1 afer all IO and data are correctly read.
c***************************************************************
