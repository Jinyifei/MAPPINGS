...
c
  ...
  120 format ( i2,',', a6,',',1pg9.1,',',1pg14.6)
       ...
       write (*,90)
       do j=1,nl
        write (luop,120) j,nfmterm(j,idx),wim(j,idx),eim(j,1,idx)/(plk*
     &   cls)
        write (*,100) j,nfmterm(j,idx),wim(j,idx),eim(j,1,idx)/(plk*cls)
       enddo
      endif
c
...
c
     ...
     &   fmlam(itr,idx),itr=1,nt)
        write (*,'(/,a,38(x,1pg14.7," "))') 'T,  ne,',(1.0d8*fmlam(itr,
     &   idx),itr=1,nt)
       endif
       if (linetype.eq.2) then
  ...
  350 format (i2,i2,3(x,1pe11.4))
        ...
        if (linetype.eq.0) then
         ...
         do itr=1,nt
          i=nfmlower(itr,idx)
          j=nfmupper(itr,idx)
          write (luop,340) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,aim(j,i,
     &     idx)
          write (*,350) i,j,eim(i,j,idx),fmlam(itr,idx)*1.d8,aim(j,i,
     &     idx)
         enddo
        endif
        if (linetype.eq.2) then
         ...
         do itr=1,nt
          i=nfelower(itr,idx)
          j=nfeupper(itr,idx)
          write (luop,340) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,aife(j,
     &     i,idx)
          write (*,350) i,j,eife(i,j,idx),felam(itr,idx)*1.d8,aife(j,i,
     &     idx)
         enddo
        endif
c
...
c
   ...
   80   format ( '% ',i2,',', a6,',',i2.2,',',1pg14.6)
     ...
     &   cls)
        write (*,60) j,nfmterm(j,idx),wim(j,idx),eim(j,1,idx)/(plk*cls)
       enddo
      endif
c
...
c
  ...
  170 format
     &('% ',i2,',',i2,5(',',1pg14.7),', 'i2.2,', ',i2.2,',',1pg12.5)
       ...
       write (luop,170) i,j,eim(i,j,idx),fmlam(itr0,idx)*1.d8,ups0,a0,f,
...
c
  ...
  190  format(a16,a16,2(', ',1pg14.7),a16,2(a48,a32))
       write (luop,190) '%log10 ne (cm-3)',', log10 T (K)   ',lam0,lam1,
     &  ',   ratio 2/1   ',',      alpha12_0,      alpha21_0,          A
     &21_0',',         pop1_0,         pop2_0',',      alpha12_1,      a
     &lpha21_1,          A21_1',',         pop1_1,         pop2_1'
c
       write (*,190) '%log10 ne (cm-3)',', log10 T (K)   ',lam0,lam1,', 
     &  ratio 2/1   ',',      alpha12_0,      alpha21_0,          A21_0'
     &  ,',         pop1_0,      pop2_0',',      alpha12_1,      alpha21
     &_1,          A21_1',',         pop1_1,         pop2_1'
c
...
c
  200     format('  ',1pg14.7,14(', ',1pg14.7))
     ...
     &   fmx(nfmlower(itr1,idx),idx),fmx(nfmupper(itr1,idx),idx)
        write (*,200) dlog10(de),dlog10(t),(br(itr),itr=1,3),colexrate0,
     &   coldexrate0,a0,fmx(nfmlower(itr0,idx),idx),fmx(nfmupper(itr0,
     &   idx),idx),colexrate1,coldexrate1,a1,fmx(nfmlower(itr1,idx),idx)
...
c
  ...
  180 format
     &('% ',i2,',',i2,5(',',1pg14.7),', ',i2.2,', ',i2.2,',',1pg12.5)
       ...
       a0=aim(j,i,idx)
       ncrit0=(a0*wupper)/(rka*f*ups0)
       write (luop,180) i,j,eim(i,j,idx),fmlam(itr0,idx)*1.d8,ups0,a0,f,
     &  wlower,wupper,dlog10(ncrit0)
     ...
     &  wlower,wupper,dlog10(ncrit0)
       ...
       a1=aim(j,i,idx)
       ncrit1=(a1*wupper)/(rka*f*ups1)
       write (luop,180) i,j,eim(i,j,idx),fmlam(itr1,idx)*1.d8,ups1,a1,f,
     ...
     &  wlower,wupper,dlog10(ncrit1)
       ...
       write (luop,180) i,j,eim(i,j,idx),fmlam(itr2,idx)*1.d8,ups2,a2,f,
...
c
  ...
  210     format('  ',1pg14.7,20(', ',1pg14.7))
     ...
     &   fmx(nfmupper(itr2,idx),idx)
        write (*,210) dlog10(t),dlog10(de),(br(itr),itr=1,4),colexrate0,
     &   coldexrate0,a0,fmx(nfmlower(itr0,idx),idx),fmx(nfmupper(itr0,
     &   idx),idx),colexrate1,coldexrate1,a1,fmx(nfmlower(itr1,idx),idx)
     &   ,fmx(nfmupper(itr1,idx),idx),colexrate2,coldexrate2,a2,
     &   fmx(nfmlower(itr2,idx),idx),fmx(nfmupper(itr2,idx),idx)
       enddo
       close (luop)
c
c Fixed Te
