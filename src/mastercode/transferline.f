       ...
       do line=1,nhlines
        ...
        if (j.ne.0) then
         if ((z*p.ge.pzlimit).and.(series.eq.1)) then
          ...
          hydlin(2,line,series)=fdismul(telc,dh,dr_eff,dvemh,atom,ion,
     &     es,f)
         endif
        endif
       enddo
       ...
       do line=1,nhelines
        ...
        if (j.ne.0) then
         if ((z*p.ge.pzlimit).and.(series.eq.1)) then
          ...
          f=hydrogf(line,series)
          hellin(2,line,series)=fdismul(telc,dh,dr_eff,dvemh,atom,ion,
     &     es,f)
         endif
        endif
       enddo
       ...
       if (xr3lines_uplin(1,line).gt.epsilon) then
        if (xr3lines_frac(line).gt.epsilon) then
         ...
         if (z*p.ge.pzlimit) then
          f=xr3lines_gf(line)
          es=xr3lines_egij(line)
          xr3lines_uplin(2,line)=fdismul(tup,dh,druph,dvuph,atom,ion,es,
     &     f)
         endif
        endif
       endif
       ...
       if (xrllines_uplin(1,line).gt.epsilon) then
        if (xrllines_frac(line).gt.epsilon) then
         ...
         if (z*p.ge.pzlimit) then
          f=xrllines_gf(line)
          es=xrllines_egij(line)
          xrllines_uplin(2,line)=fdismul(tup,dh,druph,dvuph,atom,ion,es,
     &     f)
         endif
        endif
       endif
      ...
      do series=1,nhseries
       do line=1,nhlines
        ...
        if ((z*p.ge.pzlimit).and.(series.eq.1)) then
         es=(lmev/hlambda(line,series))*ev
         f=hydrogf(line,series)
         hyduplin(2,line,series)=fdismul(tup,dh,dr_eff,dvuph,atom,ion,
     &    es,f)
        endif
       enddo
      enddo
      ...
      do series=1,nheseries
       do line=1,nhelines
        ...
        if ((z*p.ge.pzlimit).and.(series.eq.1)) then
         es=(lmev/helambda(line,series))*ev
         f=hydrogf(line,series)
         heluplin(2,line,series)=fdismul(tup,dh,dr_eff,dvuph,atom,ion,
     &    es,f)
        endif
       enddo
      enddo
       ...
       if (xr3lines_dwlin(1,line).gt.epsilon) then
        if (xr3lines_frac(line).gt.epsilon) then
         ...
         if ((z*p.ge.pzlimit)) then
          f=xr3lines_gf(line)
          es=xr3lines_egij(line)
          xr3lines_dwlin(2,line)=fdismul(tdw,dh,drdwh,dvdwh,atom,ion,es,
     &     f)
         endif
        endif
       endif
       ...
       if (xrllines_dwlin(1,line).gt.epsilon) then
        if (xrllines_frac(line).gt.epsilon) then
         ...
         if ((z*p.ge.pzlimit)) then
          f=xrllines_gf(line)
          es=xrllines_egij(line)
          xrllines_dwlin(2,line)=fdismul(tdw,dh,drdwh,dvdwh,atom,ion,es,
     &     f)
         endif
        endif
       endif
      ...
      do series=1,nhseries
       do line=1,nhlines
        ...
        if ((z*p.ge.pzlimit).and.(series.eq.1)) then
         f=hydrogf(line,series)
         es=(lmev/hlambda(line,series))*ev
         hyddwlin(2,line,series)=fdismul(tdw,dh,dr_eff,dvdwh,atom,ion,
     &    es,f)
        endif
       enddo
      enddo
...
c             if (heldwlin(1,line,series).gt.epsilon) then
         f=hydrogf(line,series)
         es=(lmev/helambda(line,series))*ev
         heldwlin(2,line,series)=fdismul(tdw,dh,dr_eff,dvdwh,atom,ion,
     &    es,f)
c             endif
        endif
       enddo
...
c
       if (aboiii>1.d-6) then
        ...
        taula=2.0d6*abheii
        if (vra2.gt.0.01d0) taula=taula*(erf(vra2)/vra2)*0.8862564664d0
c
        atom=zmap(1)
