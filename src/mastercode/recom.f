...
c
        ...
        if (rrtype.gt.0) then
         ...
         if (rrtype.eq.4) then
          tt3=dsqrt(tp/a3)
          tt4=dsqrt(tp/a4)
          rate=a1/(tt3*((tt3+1.d0)**(1.d0-a2))*((tt4+1.d0)**(1.d0+a2)))
          goto 10
         endif
         if (rrtype.eq.6) then
          a5=arrec(5,ir)
          ...
          rate=a1/(tt4*((tt4+1.d0)**(1.d0-tt6))*((tt5+1.d0)**(1.d0+tt6))
     &     )
         endif
        endif
c
...
c
        ...
        if ((mapz(atom).eq.2).and.(ion.eq.2).and.(recommode.eq.0)) goto
     &   40
c
        drtype=typedrec(ir)
        do i=1,8
   ...
   30     continue
         ...
         if (drtype.eq.2) then
          drate=t32*(co(1)*dexp(-co(3)/tp))*(1.d0+co(2)*dexp(-co(4)/tp))
         endif
         if (drtype.eq.3) then
c
