...
c
     ...
     &    tab,'rho ',tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)
     &',tab,'L/(ne.nH)',tab,(rom(j),tab,j=1,maxion(i))
        endif
        if (jnorm.eq.1) then
         write (luions(i),90) 'T ',tab,'n_e',tab,'n_H',tab,'nH^2',tab,'r
     &ho ',tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/(
     &nH^2)',tab,(rom(j),tab,j=1,maxion(i))
        endif
        if (jnorm.eq.2) then
         write (luions(i),90) 'T ',tab,'n_e',tab,'n_H',tab,'ne.ni',tab,'
     &rho ',tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/
     &(ne.ni)',tab,(rom(j),tab,j=1,maxion(i))
        endif
        if (jnorm.eq.3) then
         write (luions(i),90) 'T ',tab,'n_e',tab,'n_H',tab,'n^2',tab,'rh
     &o ',tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/(n
     &^2)',tab,(rom(j),tab,j=1,maxion(i))
        endif
        if (jnorm.eq.4) then
         write (luions(i),90) 'T ',tab,'n_e',tab,'n_H',tab,'ne^2',tab,'r
     &ho ',tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/(
     &ne^2)',tab,(rom(j),tab,j=1,maxion(i))
        endif
        write (luions(i),90) '(K)',tab,'(/cm^3)',tab,'(/cm^3)',tab,'(/cm
     &^6)',tab,'(g/cm^3)',tab,' ',tab,' ',tab,'(amu)',tab,'(erg/cm^3/s)'
...
c
  ...
  120 format(' Lambda Cooling Function and X-ray Fractions:'/
     ...
     &nH)',tab,'0.0-0.1keV',tab,'0.1-0.5keV',tab,'0.5-1.0keV',tab,'1.0-2
     ...
     &  tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/(ne 
     &ni)',tab,'0.0-0.1keV',tab,'0.1-0.5keV',tab,'0.5-1.0keV',tab,'1.0-2
     &.0keV',tab,'2.0-10.0keV'
      endif
      if (jnorm.eq.3) then
       write (luop,90) 'T ',tab,'n_e',tab,'n_H',tab,'n^2',tab,'rho ',
     &  tab,'XHI   ',tab,'XHII  ',tab,'mu ',tab,'Losses (L)',tab,'L/(n^2
     &)',tab,'0.0-0.1keV',tab,'0.1-0.5keV',tab,'0.5-1.0keV',tab,'1.0-2.0
     &keV',tab,'2.0-10.0keV'
      endif
      if (jnorm.eq.4) then
...
c
     ...
     &  b2,b3,b4
c
       write (luop,100) t,tab,de,tab,dh,tab,en,tab,rho,tab,pop(1,1),tab,
...
c
       if (jcoolelems.eq.'Y') then
        do i=1,atypes
         ...
         write (luions(i),110) t,tab,de,tab,dh,tab,n,tab,rho,tab,pop(1,
     &    1),tab,pop(2,1),tab,mu,tab,coolz(i),tab,coolz(i)*invn,(tab,
     &    coolzion(j,i)*invn,j=1,maxion(i))
         close (luions(i))
        enddo
       endif
