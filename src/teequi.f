...
c
      if (dabs(dlos).lt.delmin) goto 70
...
c
   30 if (dabs(dlos).lt.0.02d0) goto 50
      t2=tdef
      if (expertmode.gt.0) write (luty,40) dlodef,tdef
      if (ntest.eq.'Y') write (luop,40) dlodef,tdef
   40 format(' CONVERGENCE FOR EQUIL. TEMP. IS TOO SLOW   ' ,'DL:'
     &     ,1pg9.2,'  TE:',0pf9.0)
   50 continue
c
...
c
          ...
          if (((alos.lt.delexi).or.(t2.lt.tmin)).or.((jok.gt.4)
     &     .and.(alos.lt.(dble(jok)*0.6d-5)))) goto 30
c
...
c
   30   if (alos.lt.0.01d0) goto 40
        if (expertmode.gt.0) then
          write (*,*) 'DLOS CONVERGENCE POOR <30K ',dl0,teinit,dlos,t2,
     &     dlodef,tdef
        endif
        t2=tdef
   40   continue
...
c
     ...
     &     .and.(dabs(dlos).lt.(dble(jok)*0.6d-5)))) goto 50
c
c didnt converge = retry...
c
          c2=arctanh(dlos)
...
c     ***IF CONVERGENCE POOR , USES TEMP. WITH MIN. ABS(DLOS)
c
   50   if (dabs(dlos).lt.0.01d0) goto 60
        if (expertmode.gt.0) then
          write (*,*) 'TDLOS CONVERGENCE POOR <30K ',dl0,teinit,dlos,t2,
     &     dlodef,tdef
        endif
        t2=tdef
   60   continue
